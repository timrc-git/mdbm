...
// void DupReplaceTestBase::V2ToV3Replace() {
//   // callers of remap() v2 -- error on  mdbm.c:2017
//   //    mdbm_open() - should be harmless
//   //    mdbm_fetch_internal()
//   //    _write_access_check()
//   //    getnext()
//   //
//   {
//     int flags = MDBM_O_RDWR | MDBM_OPEN_NOLOCK;
//     MakeTestMdbm(file1, MDBM_CREATE_V2);
//     MakeTestMdbm(file2, MDBM_CREATE_V3);
//     MdbmHolder db = mdbm_open(file1.c_str(), flags, 0644, 1024, 4096);
//     MdbmHolder db2 = mdbm_open(file2.c_str(), flags, 0644, 1024, 4096);
//     CPPUNIT_ASSERT(0 != mdbm_replace_db(db, file2.c_str()));
//   }
//   int ret;
//   int reps = 30;
//   // synchronized replace with excl/part/nolock for delete/store/fetch
//   for (int i=0; i<reps; ++i) {
//     int flags = MDBM_O_RDWR;
//     char key[4096];
//     char val[4096];
//     datum kdat, vdat;
//
...
//     kdat.dsize = sprintf(key, "key %04d ==val", i);
//     kdat.dptr = key;
//     vdat.dsize = sprintf(val, "whereas this is value %07d pid:%d", i, getpid());
//     vdat.dptr = val;
//     mdbm_store(db, kdat, vdat, MDBM_REPLACE);
//
//     if (1 == (i%3)) {
...
//   }
//
//   //   a large number random replaces with mixed of the above ops
//   for (int i=0; i<500; ++i) {
//     int flags = MDBM_O_RDWR;
//     int j, ops = 100;
//     int targ = lrand48() % ops;
//     char key[4096];
//     char val[4096];
//     datum kdat, vdat;
//
...
//     MdbmHolder db2 = mdbm_open(file2.c_str(), flags, 0644, 1024, 4096);
//
//     // store/fetch/delete some values
//     for (j=10; j<ops; ++j) {
//       if ((j == targ) && (0 == (i%3))) {
//         CPPUNIT_ASSERT(0 == mdbm_replace_db(db, file2.c_str()));
//       }
//       kdat.dsize = sprintf(key, "key %04d ==val", j);
//       kdat.dptr = key;
//       vdat.dsize = sprintf(val, "whereas this is value %07d pid:%d", j, getpid());
//       vdat.dptr = val;
//       mdbm_store(db, kdat, vdat, MDBM_REPLACE);
//       if ((j == targ) && (1 == (i%3))) {
//         CPPUNIT_ASSERT(0 == mdbm_replace_db(db, file2.c_str()));
...
//void DupReplaceTestBase::V2ToV3ReplaceND() {
//  // Non-deterministic (asynchronous) replace test
//
//  int status;
//  int REPLACE = 1;
//  int WAITING =-1;
//
//  void* data = mmap(NULL, 4096, PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
//  CPPUNIT_ASSERT(data);
//  memset(data, 0, 4096);
//  Synchro& synch = *(Synchro*)data;
//  synch.Init();
//
//  // fork a proc that does the replace asynchronously
//  //pid_t ppid = getpid();
//  pid_t pid = fork();
...
//    int count = 0;
//    // CHILD
//    // loop waiting for the sign to replace the db
//    while (!synch.shouldQuit) {
//      // random sleep
//      int sleepTime = lrand48() % 10000;
//      usleep(sleepTime);
//
//      synch.Lock();
//      int replaceFlag = synch.flag;
//      synch.UnLock();
//
//      if (replaceFlag == REPLACE) {
//        int flags = MDBM_O_RDWR;
//        //fprintf(stderr, "ND child open %d\n", count);
//        {
//          MdbmHolder db = mdbm_open(file1.c_str(), flags, 0644, 1024, 4096);
//          if (NULL == (MDBM*)db) {
//            fprintf(stderr, "  ND child failed to open mdbm %s\n", file1.c_str());
//          }
//          CPPUNIT_ASSERT(NULL != (MDBM*)db);
//          //fprintf(stderr, "  ND child Performing replace %d\n", count);
//          int ret = mdbm_replace_db(db, file2.c_str());
//          if (ret != 0) {
//            fprintf(stderr, "  ND Performing replace %d FAILED!: return code %d\n", count, ret);
//            synch.didQuit = 1;
//            exit(-1);
//          }
//          //fprintf(stderr, "  ND child replace %d done\n", count);
//          synch.Lock();
//          synch.flag = WAITING; // signal parent
//          synch.UnLock();
//          //fprintf(stderr, "  ND child signalled parent %d \n", count);
//        }
//        //fprintf(stderr, "  ND child db closed %d \n", count);
//        ++count;
//      }
//    }
//    fprintf(stderr, "ND child exiting\n");
//    munmap(data, 4096);
//    exit(0);
//  }
//
//  // PARENT
//  // perform random operations and signal for replace occasionally
//  char key[4096];
//  char val[4096];
//  memset(key, 0, 4096); // keep valgrind happy
//  memset(val, 0, 4096);
//
...
//    bool large = (0 == i%4);
//
//    if (0 == (i%100)) { fprintf(stderr, "ND replace iteration %d\n", i); }
//
//    int cflags = 0;
//    if (large) {
//      cflags |= MDBM_LARGE_OBJECTS;
//    }
//    MakeTestMdbm(file1, MDBM_CREATE_V2 | cflags);
//    MakeTestMdbm(file2, MDBM_CREATE_V3 | cflags);
//    MdbmHolder db = mdbm_open(file1.c_str(), flags, 0644, 1024, 4096);
//    //fprintf(stderr, "ND Flagging REPLACE\n");
//
//    synch.Lock();
//    synch.flag = REPLACE; // signal child to replace
//    synch.UnLock();
//
//    while (1) {
//      if (synch.didQuit) {
...
//        // abnormal child exit...
//        fprintf(stderr, "ND child exited abnormally\n");
//        synch.didQuit = -1;
//      }
//    }
//    //fprintf(stderr, "ND Flagging WAIT\n");
//    if (synch.didQuit) {
//      break;
//    }
//  }
//  //fprintf(stderr, "ND Signalling QUIT\n");
//  synch.shouldQuit = true; // signal child to exit
//  int didQuit = synch.didQuit;
//  munmap(data, 4096);
//
//  // wait for child
//  fprintf(stderr, "ND waiting for child\n");
//  if (didQuit >= 0) {
//    pid_t wp_ret=0;
//    fprintf(stderr, "ND waitpid(%d) child\n", pid);
//    wp_ret = waitpid(pid, &status, 0);
//    fprintf(stderr, "ND waitpid(%d) child returned %d\n", pid, wp_ret);
//    CPPUNIT_ASSERT(wp_ret == pid);
//  }
//  fprintf(stderr, "ND finished\n");
//  CPPUNIT_ASSERT(didQuit >= 0);
//  CPPUNIT_ASSERT(WIFEXITED(status)); // exit'ed normally
