...
// example code that performs fetch using shared locks, store/delete by getting
...
{
    string fileName = "/tmp/test_shared.mdbm";
    unlink(fileName.c_str());  // cleanup for testing purposes
    unsigned int mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH;   // 0664

    // All processes accessing an MDBM must use the same locking mode: in this case shared locking
              ...
              size_t   valueSize)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        // With shared locking, writers must get exlusive lock
